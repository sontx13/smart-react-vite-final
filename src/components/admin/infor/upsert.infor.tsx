...
import { callCreateInfor, callFetchAllSkill, callFetchApp, callFetchInforById, callUpdateInfor, callUploadSingleFile } from "@/config/api";
...
interface IInforLogo {
    name: string;
    uid: string;
}

const ViewUpsertInfor = (props: any) => {
    ...
    const id = params?.get("id"); // infor id
    const [dataUpdate, setDataUpdate] = useState<IInfor | null>(null);
    const [form] = Form.useForm();

    const [loadingUpload, setLoadingUpload] = useState<boolean>(false);

    const [dataIcon, setDataIcon] = useState<IInforLogo[]>([]);
   ...
   const app = useAppSelector(state => state.account.user.app);
    ...
    useEffect(() => {
        const init = async () => {
            ...
            if (id) {
                const res = await callFetchInforById(id);
                ...
                if (res && res.data) {
                    ...
                    setFileList([{
                        ...
                        name: res.data.icon,
                        status: 'done',
                        url: `${import.meta.env.VITE_BACKEND_URL}/storage/infor/${res.data.icon}`,
                        thumbUrl: `${import.meta.env.VITE_BACKEND_URL}/storage/infor/${res.data.icon}`
                    }]);
    ...
    const onFinish = async (values: any) => {
        ...
        if (dataUpdate?.id) {
          // ✅ Update mode
          res = await callUpdateInfor(infor, dataUpdate.id);
          if (res?.data) {
            message.success("Cập nhật infor thành công");
          ...
          // ✅ Create mode
          res = await callCreateInfor(infor);
          if (res?.data) {
            message.success("Tạo mới infor thành công");
            navigate('/admin/infor');
          } else {
            notification.error({
              message: 'Có lỗi xảy ra',
    ...
    const handleRemoveFile = (file: any) => {
        ...
        setDataIcon([]);        // clear dữ liệu logo backend
    ...
    const getBase64 = (img: any, callback: any) => {
        ...
        const handleUploadFileLogo = async ({ file, onSuccess, onError }: any) => {
            try {
              const res = await callUploadSingleFile(file, "infor");
              if (res && res.data) {
                const newFile = {
                  uid: uuidv4(),
                  name: res.data.fileName,
                  status: 'done',
                  url: `${import.meta.env.VITE_BACKEND_URL}/storage/infor/${res.data.fileName}`,
                  thumbUrl: `${import.meta.env.VITE_BACKEND_URL}/storage/infor/${res.data.fileName}`
                };
          
                setFileList([newFile]); // cập nhật fileList UI
                setDataIcon([{ name: res.data.fileName, uid: newFile.uid }]); // lưu vào state
        ...
        <div className={styles["upsert-container"]}>
            <div className={styles["title"]}>
                <Breadcrumb
                    ...
                    items={[
                        {
                            title: <Link to="/admin/infor">Quản lý Thông tin trợ giúp</Link>,
                        },
                        {
                            title: 'Cập nhật Thông tin trợ giúp',
                        },
                    ]}
                />
            </div>
                        ...
                        submitter={
                            {
                                searchConfig: {
                                    resetText: "Hủy",
                                    submitText: <>{dataUpdate?.id ? "Cập nhật Infor" : "Tạo mới Infor"}</>
                                },
                                onReset: () => navigate('/admin/infor'),
                    ...
                    >
                        ...
                        <Row gutter={[20, 20]}>
                            ...
                            <Modal
                                ...
                                style={{ zIndex: 1500 }}
                            >
                                <img alt="inforple" style={{ width: '100%' }} src={previewImage} />
                            </Modal>
...
export default ViewUpsertInfor;