...
import { callCreateCategory, callFetchAllSkill, callFetchApp, callFetchCategoryById, callUpdateCategory, callUploadSingleFile } from "@/config/api";
...
const ViewUpsertCategory = (props: any) => {
    ...
    const id = params?.get("id"); // category id
    const [dataUpdate, setDataUpdate] = useState<ICategory | null>(null);
    const [form] = Form.useForm();

    const [loadingUpload, setLoadingUpload] = useState<boolean>(false);

    const [dataLogo, setDataLogo] = useState<ICategoryLogo[]>([]);
            ...
            if (id) {
                ...
                if (res && res.data) {
                    ...
                    setFileList([{
                        ...
                        name: res.data.icon,
                        status: 'done',
                        url: `${import.meta.env.VITE_BACKEND_URL}/storage/category/${res.data.icon}`,
                        thumbUrl: `${import.meta.env.VITE_BACKEND_URL}/storage/category/${res.data.icon}`
                    }]);
    ...
    const onFinish = async (values: any) => {
        ...
        if (dataUpdate?.id) {
          // ✅ Update mode
          res = await callUpdateCategory(category, dataUpdate.id);
          if (res?.data) {
          ...
          // ✅ Create mode
          res = await callCreateCategory(category);
          if (res?.data) {
    ...
    const getBase64 = (img: any, callback: any) => {
        ...
        const handleUploadFileLogo = async ({ file, onSuccess, onError }: any) => {
            try {
              const res = await callUploadSingleFile(file, "category");
              if (res && res.data) {
                const newFile = {
                  uid: uuidv4(),
                  name: res.data.fileName,
                  status: 'done',
                  url: `${import.meta.env.VITE_BACKEND_URL}/storage/category/${res.data.fileName}`,
                  thumbUrl: `${import.meta.env.VITE_BACKEND_URL}/storage/category/${res.data.fileName}`
                };
        ...
        <div className={styles["upsert-container"]}>
            <div className={styles["title"]}>
                <Breadcrumb
                    ...
                    items={[
                        {
                            title: <Link to="/admin/category">Quản lý Chuyên mục</Link>,
                        },
                        {
                        ...
                        submitter={
                            {
                                searchConfig: {
                                    resetText: "Hủy",
                                    submitText: <>{dataUpdate?.id ? "Cập nhật Category" : "Tạo mới Category"}</>
                                },
                                onReset: () => navigate('/admin/category'),
                    ...
                    >
                        ...
                        <Row gutter={[20, 20]}>
                            ...
                            <Modal
                                ...
                                style={{ zIndex: 1500 }}
                            >
                                <img alt="categoryple" style={{ width: '100%' }} src={previewImage} />
                            </Modal>
...
export default ViewUpsertCategory;