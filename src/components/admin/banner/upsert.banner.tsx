...
import { callCreateBanner, callFetchAllSkill, callFetchApp, callFetchBannerById, callUpdateBanner, callUploadSingleFile } from "@/config/api";
import ReactQuill from 'react-quill';
import 'react-quill/dist/quill.snow.css';
import { CheckSquareOutlined, LoadingOutlined, PlusOutlined } from "@ant-design/icons";
import enUS from 'antd/lib/locale/en_US';
import { IBanner } from "@/types/backend";
...
const ViewUpsertBanner = (props: any) => {
    ...
    const id = params?.get("id"); // banner id
    const [dataUpdate, setDataUpdate] = useState<IBanner | null>(null);
    const [form] = Form.useForm();

    const [loadingUpload, setLoadingUpload] = useState<boolean>(false);

    const [dataImage, setDataImage] = useState<IBannerImage[]>([]);
    ...
    useEffect(() => {
        const init = async () => {
            ...
            if (id) {
                const res = await callFetchBannerById(id);
                ...
                if (res && res.data) {
                    ...
                    setFileList([{
                        ...
                        name: res.data.image,
                        status: 'done',
                        url: `${import.meta.env.VITE_BACKEND_URL}/storage/banner/${res.data.image}`,
                        thumbUrl: `${import.meta.env.VITE_BACKEND_URL}/storage/banner/${res.data.image}`
                    }]);
    ...
    const onFinish = async (values: any) => {
        ...
        if (dataUpdate?.id) {
          // ✅ Update mode
          res = await callUpdateBanner(banner, dataUpdate.id);
          if (res?.data) {
          ...
          // ✅ Create mode
          res = await callCreateBanner(banner);
          if (res?.data) {
            message.success("Tạo mới banner thành công");
            navigate('/admin/banner');
          } else {
            notification.error({
              message: 'Có lỗi xảy ra',
            ...
            try {
              ...
              if (res && res.data) {
                const newFile = {
                  ...
                  status: 'done',
                  url: `${import.meta.env.VITE_BACKEND_URL}/storage/banner/${res.data.fileName}`,
                  thumbUrl: `${import.meta.env.VITE_BACKEND_URL}/storage/banner/${res.data.fileName}`
                };
        ...
        <div className={styles["upsert-container"]}>
            <div className={styles["title"]}>
                <Breadcrumb
                    ...
                    items={[
                        {
                            title: <Link to="/admin/banner">Quản lý Banner</Link>,
                        },
                        {
                        ...
                        submitter={
                            {
                                searchConfig: {
                                    resetText: "Hủy",
                                    submitText: <>{dataUpdate?.id ? "Cập nhật Banner" : "Tạo mới Banner"}</>
                                },
                                onReset: () => navigate('/admin/banner'),
                    ...
                    >
                        ...
                        <Row gutter={[20, 20]}>
                            ...
                            <Modal
                                ...
                                style={{ zIndex: 1500 }}
                            >
                                <img alt="bannerple" style={{ width: '100%' }} src={previewImage} />
                            </Modal>
...
export default ViewUpsertBanner;