...
import { fetchHotline } from "@/redux/slice/hotlineSlide";
...
import { callDeleteHotline, callFetchApp } from "@/config/api";
...
const HotlinePage = () => {
    ...
    const [dataInit, setDataInit] = useState<IHotline | null>(null);
    

    const tableRef = useRef<ActionType>();

    const isFetching = useAppSelector(state => state.hotline.isFetching);
    const meta = useAppSelector(state => state.hotline.meta);
    const apps = useAppSelector(state => state.hotline.result);
    ...
    const handleDeleteHotline = async (id: string | undefined) => {
        if (id) {
            const res = await callDeleteHotline(id);
            if (res && +res.statusCode === 200) {
                message.success('Xóa Hotline thành công');
                reloadTable();
            } else {
                notification.error({
    ...
    const columns: ProColumns<IHotline>[] = [
        {
            title: 'STT',
            key: 'index',
            width: 50,
        ...
        },
       {
        ...
        },
        {
            ...
            render: (_value, entity, _index, _action) => (
                <Space>
                    < Access
                        permission={ALL_PERMISSIONS.HOTLINES.UPDATE}
                        hideChildren
                    >
                        <EditOutlined
                             style={{
                            ...
                            type=""
                            onClick={() => {
                                navigate(`/admin/hotline/upsert?id=${entity.id}`)
                            }}
                        />
                    </Access >
                    <Access
                        permission={ALL_PERMISSIONS.HOTLINES.DELETE}
                        hideChildren
                    >
                        <Popconfirm
                            placement="leftTop"
                            title={"Xác nhận xóa hotline"}
                            description={"Bạn có chắc chắn muốn xóa hotline này ?"}
                            onConfirm={() => handleDeleteHotline(entity.id)}
                            okText="Xác nhận"
                            cancelText="Hủy"
                        >
    ...
    return (
        <div>
            <Access
                permission={ALL_PERMISSIONS.HOTLINES.GET_PAGINATE}
            >
                <DataTable<IHotline>
                    actionRef={tableRef}
                    headerTitle="Danh sách đường dây nóng"
                    rowKey="id"
                    loading={isFetching}
                    columns={columns}
                    dataSource={apps}
