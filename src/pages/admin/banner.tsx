...
import { fetchBanner } from "@/redux/slice/bannerSlide";
...
import { callDeleteBanner, callFetchApp } from "@/config/api";
...
const BannerPage = () => {
    ...
    const [dataInit, setDataInit] = useState<IBanner | null>(null);
    

    const tableRef = useRef<ActionType>();

    const isFetching = useAppSelector(state => state.banner.isFetching);
    const meta = useAppSelector(state => state.banner.meta);
    const apps = useAppSelector(state => state.banner.result);
    ...
    const columns: ProColumns<IBanner>[] = [
        ...
        },
        {
            ...
            render: (_value, entity, _index, _action) => (
                <Space>
                    < Access
                        permission={ALL_PERMISSIONS.BANNERS.UPDATE}
                        hideChildren
                    >
                        <EditOutlined
                             style={{
                            ...
                            type=""
                            onClick={() => {
                                navigate(`/admin/banner/upsert?id=${entity.id}`)
                            }}
                        />
                    </Access >
                    <Access
                        permission={ALL_PERMISSIONS.BANNERS.DELETE}
                        hideChildren
                    >
                        <Popconfirm
                            placement="leftTop"
                            ...
                            onConfirm={() => handleDeleteBanner(entity.id)}
                            okText="Xác nhận"
                            cancelText="Hủy"
                        >
    ...
    return (
        <div>
            <Access
                permission={ALL_PERMISSIONS.BANNERS.GET_PAGINATE}
            >
                <DataTable<IBanner>
                    actionRef={tableRef}
                    headerTitle="Danh sách Banner liên kết"
                    rowKey="id"
                    loading={isFetching}
                    columns={columns}
                    dataSource={apps}
