...
const QAPage = () => {
    ...
    const [dataInit, setDataInit] = useState<IQA | null>(null);
    

    const tableRef = useRef<ActionType>();

    const isFetching = useAppSelector(state => state.qa.isFetching);
    const meta = useAppSelector(state => state.qa.meta);
    const apps = useAppSelector(state => state.qa.result);
    ...
    const handleDeleteQA = async (id: string | undefined) => {
        if (id) {
            const res = await callDeleteQA(id);
    ...
    const columns: ProColumns<IQA>[] = [
        ...
        },
        {
            ...
            render: (_value, entity, _index, _action) => (
                <Space>
                    < Access
                        permission={ALL_PERMISSIONS.QAS.UPDATE}
                        hideChildren
                    >
                        <EditOutlined
                             style={{
                    ...
                    >
                        <Popconfirm
                            placement="leftTop"
                            title={"Xác nhận xóa qa"}
                            description={"Bạn có chắc chắn muốn xóa qa này ?"}
                            onConfirm={() => handleDeleteQA(entity.id)}
                            okText="Xác nhận"
                            cancelText="Hủy"
                        >
    ...
    return (
        <div>
            <Access
                permission={ALL_PERMISSIONS.QAS.GET_PAGINATE}
            >
                <DataTable<IQA>
                    actionRef={tableRef}
                    headerTitle="Danh sách hỏi đáp"
                    rowKey="id"
                    loading={isFetching}
                    columns={columns}
                    dataSource={apps}
