...
import { fetchConfig } from "@/redux/slice/configSlide";
...
import { callDeleteConfig, callFetchApp } from "@/config/api";
...
const ConfigPage = () => {
    ...
    const [dataInit, setDataInit] = useState<IConfig | null>(null);
    

    const tableRef = useRef<ActionType>();

    const isFetching = useAppSelector(state => state.config.isFetching);
    const meta = useAppSelector(state => state.config.meta);
    const apps = useAppSelector(state => state.config.result);
    ...
    const handleDeleteConfig = async (id: string | undefined) => {
        if (id) {
            const res = await callDeleteConfig(id);
    ...
    const columns: ProColumns<IConfig>[] = [
        {
            title: 'STT',
            key: 'index',
            width: 50,
        ...
        },
        {
            ...
            render: (_value, entity, _index, _action) => (
                <Space>
                    < Access
                        permission={ALL_PERMISSIONS.CONFIGS.UPDATE}
                        hideChildren
                    >
                        <EditOutlined
                             style={{
                            ...
                            type=""
                            onClick={() => {
                                navigate(`/admin/config/upsert?id=${entity.id}`)
                            }}
                        />
                    </Access >
                    <Access
                        permission={ALL_PERMISSIONS.CONFIGS.DELETE}
                        hideChildren
                    >
                        <Popconfirm
                            placement="leftTop"
                            title={"Xác nhận xóa config"}
                            description={"Bạn có chắc chắn muốn xóa config này ?"}
                            onConfirm={() => handleDeleteConfig(entity.id)}
                            okText="Xác nhận"
                            cancelText="Hủy"
                        >
    ...
    return (
        <div>
            <Access
                permission={ALL_PERMISSIONS.CONFIGS.GET_PAGINATE}
            >
                <DataTable<IConfig>
                    actionRef={tableRef}
                    headerTitle="Danh sách cấu hình hệ thống"
                    rowKey="id"
                    loading={isFetching}
                    columns={columns}
                    dataSource={apps}
